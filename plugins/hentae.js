...
ezra({
  ...
  try {
    ...
    const cap = `*Screenshot taken by TEDDY-XMD*`;
...
ezra({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by TEDDY-XMD*`;
  ...
  function sendImage(error, results) {
    ...
    for (let i = 0; i < Math.min(results.length, 5); i++) {
      zk.sendMessage(dest, {
        ...
        caption: `*Downloaded by TEDDY-XMD${conf.BOT}*`,
        contextInfo: {
          externalAdReply: {
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and TEDDY-XMD will extract the text for you.");
    }
    ...
    message.sendMessage(user, {
      ...
      'caption': "*powered by TEDDY-XMD TECH*"
    }, {
      'quoted': messageInstance
    });
  } catch (error) {
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and TEDDY-XMD will generate it.");
    }
    const prompt = args.join(" ");
